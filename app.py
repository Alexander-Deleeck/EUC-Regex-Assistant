...
def create_input_section(field_key, label, icon):
    ...
    for i, (example, description) in enumerate(st.session_state[field_key]):
        col1, col2, col3 = st.columns([2, 4, 0.3])
        with col1:
...
def main():
    ...
    # Create main columns
    left_col, middle_col, right_col = st.columns([5, 0.8, 5])

    with left_col:
        # Top 2/3 section - Pattern creation
        # Add custom CSS for right column background
        ...
        with st.container():
            ...
            pattern_not_examples = create_input_section('pattern_not_examples', "No Match", "❌")
            st.divider()
